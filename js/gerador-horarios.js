...
(function () {
  ...
  function sundayWouldBreakWeek(sundayStoreIds, active) {
    // Única razón para bloquear un domingo: no hay NINGUNA candidata disponible.
    // El algoritmo de folgas ya garantiza los días compensatorios sin romper semana.
  ...
  function buildSchedule(active) {
    // Asignación base: cada persona va a su tienda fija o a la primera que conoce.
    // La redistribución por máximos/mínimos ocurre en intelPass.
    PEOPLE.forEach(p => {
      S.schedule[p.id] = {};
  ...
  function intelPass(active) {
    // Días preferidos para reducir a mínimos en avenida/mercado (martes y jueves)
    const PREFER_MIN_DAYS = ['TER', 'QUI'];
    ...
    DAYS.forEach(day => {
      ...
      if (ed?.type === 'work' && ca?.type === 'work' && ed.store === ca.store) {
        ...
        // Solo mover a Carla si su destino no supera el máximo
        const alt = S.openStores.find(id => {
          if (id === edStore) return false;
      ...
      // ── PASO 1: APLICAR MÁXIMOS (inamovibles) ──
      // Maxx y Shana: máximo absolutamente respetado, nunca se supera.
      // Avenida y Mercado: también se respeta, pero el exceso se redistribuye.
      STORES.filter(st => storeOpen(st.id, day)).forEach(st => {
        const max = storeMax(st.id);
        if (max === Infinity) return;
        const overWorkers = wk().filter(p => S.schedule[p.id][day].store === st.id);
        if (overWorkers.length <= max) return;

        // Excedentes: primero salen los sin tienda fija aquí, luego los de tienda fija
        const toMove = [...overWorkers]
          .sort((a, b) => {
            const aFixed = (a.store === st.id) ? 1 : 0;
            const bFixed = (b.store === st.id) ? 1 : 0;
            if (aFixed !== bFixed) return aFixed - bFixed; // sin fija salen primero
        ...
        toMove.forEach(p => {
          ...
          if (dest) {
            ...
            S.decisions.push({ type: 'info', text: `${day}: ${p.name} → ${sname(dest)} (máximo ${max} em ${sname(st.id)}).` });
          } else {
            S.alerts.push({ type: 'amber', text: `${day}: ${sname(st.id)} excede máximo (${overWorkers.length}/${max}) — sem destino para ${p.name}.` });
          }
        });
      });

      // ── PASO 2: CUBRIR MÍNIMOS ──
      // Prioridad: Avenida → Mercado → Shana → Maxx
      // Solo se mueven personas sin tienda fija; si no hay, se usan las de tienda fija SOLO como último recurso.
      STORES.filter(st => storeOpen(st.id, day)).sort((a, b) => a.priority - b.priority).forEach(st => {
        ...
        for (let i = 0; i < needed; i++) {
          if (have() >= min) break;
          // Candidatas: primero sin tienda fija, luego con tienda fija en otra tienda
          const pool = wk()
            .filter(p => {
              if (S.schedule[p.id][day].store === st.id) return false;
              if (!p.knows.includes(st.id)) return false;
              // No superar el máximo del destino (st.id)
              if (have() + 1 > storeMax(st.id)) return false;
              // No dejar la tienda de origen bajo su mínimo
              const originStore = S.schedule[p.id][day].store;
              const originCount = wk().filter(x => S.schedule[x.id][day].store === originStore).length;
              if (originCount - 1 < storeMin(originStore)) return false;
              return true;
            })
            .sort((a, b) => {
              // Sin tienda fija primero; con tienda fija en esta tienda segundo; en otra tienda: último recurso
              const scoreA = !a.store ? 0 : a.store === st.id ? 1 : 2;
              const scoreB = !b.store ? 0 : b.store === st.id ? 1 : 2;
              if (scoreA !== scoreB) return scoreA - scoreB;
              // Entre iguales: la que viene de la tienda más llena
              const ac = wk().filter(x => S.schedule[x.id][day].store === S.schedule[a.id][day].store).length;
              const bc = wk().filter(x => S.schedule[x.id][day].store === S.schedule[b.id][day].store).length;
              return bc - ac;
            })[0];
          if (pool) {
            S.schedule[pool.id][day].store = st.id;
            S.decisions.push({ type: 'warn', text: `${day}: ${pool.name} → ${sname(st.id)} (mínimo).` });
          } else {
      ...
      // ── PASO 3: REDISTRIBUCIÓN EXCEDENTES (solo Avenida↔Mercado) ──
      // Maxx y Shana ya están en su máximo. Solo mover excedentes entre Avenida y Mercado.
      // Preferir hacerlo en martes y jueves si hay que bajar a mínimos.
      // NO mover personas con tienda fija fuera de su tienda.
      // Solo mover si hay una diferencia real que mejora el equilibrio.
      (() => {
        const workers = wk();
        const flexStores = ['avenida', 'mercado'].filter(id => storeOpen(id, day));
        if (flexStores.length < 2) return;

        function cov() {
          const c = {};
          flexStores.forEach(id => { c[id] = workers.filter(p => S.schedule[p.id][day]?.store === id).length; });
          return c;
        }

        for (let iter = 0; iter < workers.length; iter++) {
          const c = cov();
          // Encontrar tienda con exceso (sobre su máximo) y tienda con déficit (bajo su máximo)
          const over  = flexStores.find(id => c[id] > storeMax(id));
          const under = flexStores.find(id => c[id] < storeMax(id) && c[id] < storeMin(id));
          if (!over && !under) break; // todo dentro de límites

          const src  = over  || flexStores.reduce((a, b) => c[a] > c[b] ? a : b);
          const dest = under || flexStores.reduce((a, b) => c[a] < c[b] ? a : b);
          if (src === dest) break;
          if (c[src] <= storeMin(src)) break; // fuente no puede perder más
          if (c[dest] >= storeMax(dest)) break; // destino ya está lleno

          // Candidata: sin tienda fija, conoce ambas tiendas, no es sandra
          const cand = workers
            .filter(p => {
  ...
  function resolveSundayStores(active) {
    ...
    // Respetar la decisión del usuario. Solo sacrificar tiendas que no tienen
    // ninguna candidata disponible (físicamente imposible abrir).
    const sacrificed = [];
    let current = [...requestedSundayStores];

    while (current.length > 0) {
      const check = sundayWouldBreakWeek(current, active);
      if (!check.breaks) break;
      // Buscar la tienda de menor prioridad sin candidatas
      const toRemove = SUNDAY_SACRIFICE_ORDER.find(sid => {
        if (!current.includes(sid)) return false;
        return sundayCandidatesFor(sid, active).length < sundayMinFor(sid);
      });
      if (!toRemove) break; // no hay candidata clara para sacrificar
