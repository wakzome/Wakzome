// ══════════════════════════════════════════════════════════════
//  TAM FASHION — invoice parser  v5
//  · Triple engine cross-check
//  · Fixed column overflow (nome truncates, numbers never wrap)
//  · Manual engine override when engines disagree
// ══════════════════════════════════════════════════════════════
(function () {
  ...
  var tamCurrentResult = null;  // full cross-validate result
  var tamEngineResults = {};    // { A: resA, B: resB, C: resC }
  ...
  async function tamHandleFile(file) {
    ...
    tamCurrentResult = null; tamEngineResults = {};
  ...
  /* Apply a result object to the UI (used both on first load and on manual override) */
  ...
  function tamEngineA(allRows) {
    var tagged = allRows.map(function(tokens,idx){
      ...
      if(tokens[0]&&REF_RE.test(tokens[0]))
        return {idx,type:'REF',ref:tokens[0]};
      ...
      if(hsM){
        ...
        if(numM){
          ...
          return {idx,type:'DATA',garmentType:tn.type,name:tn.name,pieces,unitPrice,total};
        }
      }
      if(/Versandkosten|Transportation costs/i.test(joined)){
        var anzM=joined.match(/Anzahl\s+(\d+)\s+([\d.]*\d+,\d{2})/);
        if(anzM) return {idx,type:'SHIP',packages:parseInt(anzM[1]),cost:tamParseEU(anzM[2])};
      }
      if(/Zwischensumme.*Subtotal/i.test(joined)){
        var nM=joined.match(/([\d.]*\d+,\d{2})\s*$/);
        if(nM) return {idx,type:'SUBTOTAL',value:tamParseEU(nM[1])};
      }
      if(/^ZY-\d+/.test(tokens[0]||'')) return {idx,type:'INVOICENO',value:tokens[0]};
      if(joined.includes('Datum/Date')){
        var dM=joined.match(/(\d{2}\.\d{2}\.\d{4})/);
        if(dM) return {idx,type:'DATE',value:dM[1]};
      }
      return {idx,type:'OTHER'};
    });
    var rawItems=[];
  ...
  function tamEngineB(allRows){
    ...
    for(var i=0;i<allRows.length;i++){
      ...
      if(/^ZY-\d+/.test(tokens[0]||'')){tagged.push({idx:i,type:'INVOICENO',value:tokens[0]});continue;}
      if(joined.includes('Datum/Date')){
        var dM=joined.match(/(\d{2}\.\d{2}\.\d{4})/);
  ...
  /* ════════════════════════════════════════════════════════════
     ENGINE C — math-first triplet strategy (improved)
     Finds all rows where N × price = total (±0.02).
     Additional guards vs false positives:
       · pieces 1–500
       · unit price 0.01–99.99
       · total > 0
       · row must NOT be a header/footer/address line
       · if a row has multiple valid triplets, takes the one
         whose total is largest (most specific match)
     Pairs each triplet with nearest preceding REF (window 30).
  ════════════════════════════════════════════════════════════ */
  function tamEngineC(allRows){
    var NOISE_RE=/Kunden|Konto|Datum|Seite|TAM FASHION|Wakzome|Hauptsitz|IBAN|Fon|Fax|eMail|Versand|Liefer|steuer|Paket|Bruttogewicht|Netto/i;
    ...
    for(var i=0;i<allRows.length;i++){
      ...
      while((m=NUM_RE.exec(rowStr))!==null){
        ...
        // prefer match with largest total (most specific)
        if(!best||total>best.total){
          var tn=tamExtractTypeAndName(rowStr.slice(0,m.index));
          best={pieces,unitPrice,total,tn,matchIdx:m.index};
        }
      }
      if(best){
    ...
    // Assign to nearest REF (window 30, bidirectional: prefer backward, then forward)
    var rawItems=[];
    var refPositions=tagged.filter(function(t){return t.type==='REF';});

    tagged.forEach(function(row){
      if(row.type!=='DATA_C') return;
      // Find nearest REF before this row
      ...
      // If nothing backward within 30, try forward (at most 5 rows ahead — misplaced REF edge case)
      if(!nearest){
        refPositions.forEach(function(r){
      ...
      if(nearest){
        rawItems.push({ref:nearest.ref,garmentType:row.garmentType,name:row.name,
  ...
  function tamCrossValidate(resA,resB,resC){
    ...
    var mergedGrouped=allRefs.map(function(ref){
      ...
      var fromBest=({A:mapA,B:mapB,C:mapC})[engines[0].label][ref]||present[0];
      var detailParts=[];
      ...
      return Object.assign({},fromBest,{
        confidence:'CONFLICT',conflictDetail:detailParts.join(' · '),enginesCount:present.length
      });
    }).filter(Boolean);
    ...
    var shipping =engines[0].res.shipping ||resA.shipping ||resB.shipping ||resC.shipping;
    var shipPkgs =engines[0].res.shipPkgs ||resA.shipPkgs ||resB.shipPkgs ||resC.shipPkgs;
    var shipPerPiece=totalPieces>0?shipping/totalPieces:0;
    ...
    return {
      grouped:mergedGrouped,rawItems:engines[0].res.rawItems,
      totalPieces,subtotalGoods,shipping,shipPkgs,shipPerPiece,
      grandTotal:tamRound2(subtotalGoods+shipping),
      invoiceSubtotal:meta.invoiceSubtotal,
      invoiceNo:meta.invoiceNo,invoiceDate:meta.invoiceDate,
      xv:{
        confirmed,conflicts,
        fullyAgree:conflicts.length===0&&mergedGrouped.every(function(g){return g.confidence==='CONFIRMED';}),
  ...
  /* Build a result using a single engine as source of truth (for manual override) */
  function tamBuildFromEngine(label) {
    var res = tamEngineResults[label];
    if (!res) return null;
    var shipping    = res.shipping;
    var shipPkgs    = res.shipPkgs;
    var shipPerPiece= res.totalPieces>0 ? shipping/res.totalPieces : 0;
    var grouped     = res.grouped.map(function(g){
      var base=g.pieces>0?g.totalCost/g.pieces:0;
      return Object.assign({},g,{
        confidence:'CONFIRMED',
        unitPriceWithShip:tamRound2(base+shipPerPiece),
        grandTotal:tamRound2(tamRound2(base+shipPerPiece)*g.pieces)
      });
    });
    return Object.assign({},res,{
      grouped:grouped,
      xv:{
        confirmed:grouped.length,conflicts:[],fullyAgree:true,
        bestEngine:label,manualOverride:true,
        engines:['A','B','C'].map(function(l){
          var r=tamEngineResults[l];
          return {label:l,refs:r.grouped.length,units:r.totalPieces,sub:r.subtotalGoods,score:0};
        })
      }
    });
  }
  ...
  /* ════════════════════════════════════════════════════════════
     RENDER: VALIDATION BANNER  (with engine selector if conflict)
  ════════════════════════════════════════════════════════════ */
  function tamRenderValidation(r){
    ...
    var cvHtml='';
    if(xv.manualOverride){
      cvHtml='<div class="tam-vi" style="color:#2a7a2a"><em>motor seleccionado</em>'+
             '<span>✅ a usar Motor <strong>'+tamEsc(xv.bestEngine)+'</strong> (selecção manual)</span></div>';
    } else if(allOk){
      ...
      cvHtml+='<div class="tam-vi tam-engine-sel-wrap">'+
        '<em>usar motor manualmente</em>'+
        '<span class="tam-engine-btns">'+
          ['A','B','C'].map(function(l){
            var er=tamEngineResults[l];
            var refs=er?er.grouped.length:0,units=er?er.totalPieces:0;
            var active=l===xv.bestEngine?' tam-ebtn-active':'';
            return '<button class="tam-ebtn'+active+'" data-engine="'+l+'">'+
                     'Motor '+l+'<br><small>'+refs+' refs / '+units+' un</small>'+
                   '</button>';
          }).join('')+
        '</span>'+
      '</div>';
    ...
    el.innerHTML=
      '<div class="tam-vi"><em>estado</em><span>'+(allOk||xv.manualOverride?'✅ fatura correcta':'⚠️ verificar itens marcados')+'</span></div>'+
      ...
      cvHtml+
      '<div class="tam-vi"><em>transporte/unidade</em><span><strong>'+tamFmtEU(r.shipping)+'€</strong> ÷ '+r.totalPieces+' = <strong>'+tamFmtEU(r.shipPerPiece)+'€/un</strong></span></div>';
    el.className=allOk||xv.manualOverride?'ok':'err';

    // Bind manual engine buttons
    el.querySelectorAll('.tam-ebtn').forEach(function(btn){
      btn.addEventListener('click',function(){
        var label=btn.getAttribute('data-engine');
        var overrideResult=tamBuildFromEngine(label);
        if(overrideResult) tamApplyResult(overrideResult);
      });
    });
  }

  /* ════════════════════════════════════════════════════════════
     RENDER: TABLE  — fixed columns, no overflow
  ════════════════════════════════════════════════════════════ */
  function tamRenderTable(r){
    if(!r.grouped.length) return;
    var html=
      '<table class="tam-table"><colgroup>'+
      '<col class="tam-col-num">'+        // # — 30px
      '<col class="tam-col-ref">'+        // referência — 155px
      '<col class="tam-col-nome">'+       // tipo·nome — fills rest, truncates
      '<col class="tam-col-und">'+        // UND — 46px
      '<col class="tam-col-price">'+      // P.Unit/T — 90px
      '<col class="tam-col-total">'+      // Total — 90px
      '</colgroup><thead><tr>'+
      '<th class="tam-col-num">#</th>'+
      '<th class="tam-col-ref" style="text-align:left">referência</th>'+
      '<th class="tam-col-nome" style="text-align:left">tipo · nome</th>'+
      '<th class="tam-col-und">UND</th>'+
      '<th class="tam-col-price">P.Unit/T</th>'+
      '<th class="tam-col-total">Total</th>'+
      '</tr></thead><tbody>';
    ...
    r.grouped.forEach(function(g,i){
      ...
      html+='<tr'+rowCls+tooltip+'>'+
        '<td class="tam-col-num">'+(i+1)+'</td>'+
        '<td class="tam-col-ref"><strong>'+tamEsc(g.ref)+'</strong>'+badge+'</td>'+
        '<td class="tam-col-nome" title="'+tamEsc(typeNome)+'">'+tamEsc(typeNome)+'</td>'+
        '<td class="tam-col-und">'+g.pieces+'</td>'+
        '<td class="tam-col-price">'+tamFmtEU(g.unitPriceWithShip)+'</td>'+
        '<td class="tam-col-total"><strong>'+tamFmtEU(g.grandTotal)+'</strong></td>'+
        '</tr>';
    });

    html+='</tbody><tfoot>'+
      '<tr>'+
        '<td class="tam-col-num"></td>'+
        '<td class="tam-col-ref" colspan="2"><strong>subtotal mercadoria</strong></td>'+
        '<td class="tam-col-und"><strong>'+r.totalPieces+'</strong></td>'+
        '<td class="tam-col-price"></td>'+
        '<td class="tam-col-total"><strong>'+tamFmtEU(r.subtotalGoods)+'</strong></td>'+
      '</tr>'+
      '<tr class="tam-tr-ship">'+
        '<td class="tam-col-num"></td>'+
        '<td class="tam-col-ref" colspan="2">transporte · '+r.shipPkgs+' pac. × 17,50 €</td>'+
        '<td></td><td></td>'+
        '<td class="tam-col-total">'+tamFmtEU(r.shipping)+'</td>'+
      '</tr>'+
      '<tr class="tam-tr-grand">'+
        '<td class="tam-col-num"></td>'+
        '<td class="tam-col-ref" colspan="2"><strong>total geral</strong></td>'+
        '<td class="tam-col-und"><strong>'+r.totalPieces+'</strong></td>'+
        '<td class="tam-col-price"></td>'+
        '<td class="tam-col-total"><strong>'+tamFmtEU(r.grandTotal)+'</strong></td>'+
      '</tr>'+
      '</tfoot></table>';
  ...
  function tamEnsureStyles(){
    ...
    s.id='tam-xv-styles';
    s.textContent=[
      /* Table layout — fixed, columns never bleed */
      '.tam-table{table-layout:fixed !important;width:100% !important}',
      /* Column widths */
      '.tam-col-num {width:30px;text-align:center;color:#aaa;font-size:.72rem}',
      '.tam-col-ref {width:155px;text-align:left;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}',
      '.tam-col-nome{text-align:left;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;max-width:0}',
      '.tam-col-und {width:46px;text-align:center;white-space:nowrap}',
      '.tam-col-price{width:90px;text-align:right;white-space:nowrap;padding-right:10px}',
      '.tam-col-total{width:90px;text-align:right;white-space:nowrap;padding-right:10px}',
      /* Row states */
      ...
      '.tam-engine-sel-wrap{grid-column:1/-1;margin-top:4px}',
      '.tam-engine-btns{display:flex;gap:8px;margin-top:4px;justify-content:center;flex-wrap:wrap}',
      '.tam-ebtn{border:1px solid #ccc;background:#fff;padding:5px 14px;border-radius:6px;'+
                'cursor:pointer;font-family:inherit;font-size:.75rem;line-height:1.3;text-align:center;'+
                'transition:background .15s,border-color .15s}',
      '.tam-ebtn:hover{background:#f0f0f0;border-color:#888}',
