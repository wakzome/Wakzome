...
#ag-title{font-size:2.4rem;font-weight:100;text-transform:lowercase;letter-spacing:.1em;color:#000;line-height:1}
...
#ag-hero-value{font-size:3.5rem;font-weight:100;color:#000;letter-spacing:-.02em;line-height:1;transition:color .4s}
...
.ag-card-value{font-size:1.45rem;font-weight:100;color:#000;letter-spacing:-.01em}
...
.ag-btn:hover{background:#000;color:#fff;border-color:#000}
.ag-btn:active{transform:scale(0.97)}
.ag-btn-p{background:#000;color:#fff;border-color:#000}
.ag-btn-p:hover{background:#333;border-color:#333}
...
.ag-fb.active{background:#000;color:#fff;border-color:#000}
.ag-fb[data-forn="TAM"].active{background:#1565c0;border-color:#1565c0}
.ag-fb[data-forn="GIT"].active{background:#6a1b9a;border-color:#6a1b9a}
.ag-fb[data-forn="BESTSELLER"].active{background:#880e4f;border-color:#880e4f}
.ag-fb[data-forn="CHLAMYS"].active{background:#2e7d32;border-color:#2e7d32}
...
.ag-filter-btn.active{border-color:#000;background:#000;color:#fff}
.ag-filter-btn.ag-fp.active{background:#e65100;border-color:#e65100;color:#fff}
.ag-filter-btn.ag-fv.active{background:#c62828;border-color:#c62828;color:#fff}
.ag-filter-btn.ag-fg.active{background:#2e7d32;border-color:#2e7d32;color:#fff}
...
#ag-table-wrap{background:#fff;border:1px solid #e6e6e6;border-radius:14px;overflow:hidden;overflow-x:auto}
#ag-table{width:100%;border-collapse:collapse;font-family:'MontserratLight',sans-serif;font-size:.95rem;white-space:nowrap}
...
.ag-year-btn:hover{background:#000;color:#fff;border-color:#000}
.ag-year-btn.active-year{background:#000;color:#fff;border-color:#000}
...
@media(prefers-color-scheme:dark){
  #ag-app,#ag-mb,.ag-cb,.ag-card,.ag-fb-block,#ag-forn-banner,#ag-table-wrap{background:#111!important}
  #ag-readonly-banner{border-color:#e8e8e8!important;background:#1a1a1a!important}
  #ag-readonly-banner span,#ag-readonly-badge{color:#e8e8e8!important;border-color:#e8e8e8!important}
  .ag-year-btn{background:#111!important;border-color:#2a2a2a!important;color:#e8e8e8!important}
  .ag-year-btn:hover,.ag-year-btn.active-year{background:#e8e8e8!important;color:#000!important;border-color:#e8e8e8!important}
  #ag-btn-close-year{background:#111!important;border-color:#ff6b6b!important;color:#ff6b6b!important}
  #ag-btn-close-year:hover{background:#ff6b6b!important;color:#000!important}
  .ag-btn{background:#111!important;border-color:#2a2a2a!important;color:#e8e8e8!important}
  .ag-btn:hover{background:#e8e8e8!important;color:#000!important;border-color:#e8e8e8!important}
  .ag-btn-p{background:#e8e8e8!important;color:#000!important;border-color:#e8e8e8!important}
  .ag-fb{background:#111!important;border-color:#2a2a2a!important;color:#e8e8e8!important}
  .ag-filter-btn{color:#e8e8e8!important;border-color:#2a2a2a!important}
  #ag-table thead th{background:#1a1a1a!important;color:#e8e8e8!important;border-color:#2a2a2a!important}
  #ag-table tbody tr:hover{background:#1a1a1a!important}
  #ag-table td{color:#e8e8e8!important;background:#111!important;border-color:#1e1e1e!important}
  .ag-tr-v{background:rgba(198,40,40,.06)!important}
  .ag-fb-row{border-top-color:#1a1a1a!important}
  .ag-in{background:#0d0d0d!important;border-color:#2a2a2a!important;color:#e8e8e8!important}
  #ag-mo{background:rgba(0,0,0,.85)!important}
  #ag-snack{background:#e8e8e8!important;color:#000!important}
  .ag-sub-tr td{background:#1a1a1a!important;color:#e8e8e8!important}
  .ag-chip-TAM{background:#1a237e!important;color:#90caf9!important}
  .ag-chip-GIT{background:#4a148c!important;color:#ce93d8!important}
  .ag-chip-BESTSELLER{background:#880e4f!important;color:#f48fb1!important}
  .ag-chip-CHLAMYS{background:#1b5e20!important;color:#a5d6a7!important}
  .ag-card-label,.ag-fb-rl,.ag-fb-rv,.ag-hs-l,.ag-hs-v,.ag-insight,.ag-sl,.ag-ct,.ag-dl-label,.ag-dl-pct,.ag-bar-label,.ag-tdn,.ag-fb-hdr{color:#e8e8e8!important}
  .ag-fb-rv.g{color:#4caf50!important}.ag-fb-rv.r{color:#ff6b6b!important}
  #ag-empty{color:#e8e8e8!important}
  #ag-btn-edit-once{background:#111!important;border-color:#e8e8e8!important;color:#e8e8e8!important}
  #ag-btn-edit-once:hover{background:#e8e8e8!important;color:#000!important}
  #ag-btn-confirm-once{background:#111!important;border-color:#ff6b6b!important;color:#ff6b6b!important}
  #ag-btn-confirm-once:hover{background:#ff6b6b!important;color:#000!important}
  .ag-edit-once-msg{color:#e8e8e8!important}
}
`;
...
function agBindLogic() {
  ...
  function rFornBtns(){
    ...
    var fkeys=['TAM','GIT','BESTSELLER','CHLAMYS'].filter(function(k){return found[k];});
    container.innerHTML=fkeys.map(function(k){
      var cls='ag-fb'+(agForn===k?' active':'');
      return '<button class="'+cls+'" data-forn="'+k+'">'+k+'<span class="ag-fbadge" id="ag-b-'+k+'"></span></button>';
    }).join('');
    container.querySelectorAll('.ag-fb').forEach(function(btn){
      btn.addEventListener('click',function(){
        var f=this.dataset.forn;
        if(agForn===f){agForn=null;container.querySelectorAll('.ag-fb').forEach(function(b){b.classList.remove('active');});}
        else{agForn=f;container.querySelectorAll('.ag-fb').forEach(function(b){b.classList.remove('active');});this.classList.add('active');agFilter='all';document.querySelectorAll('.ag-filter-btn').forEach(function(b){b.classList.remove('active');});document.querySelector('.ag-filter-btn[data-filter="all"]').classList.add('active');}
  ...
  function rBadges(){
    var c={TAM:0,GIT:0,BESTSELLER:0,CHLAMYS:0};
    agF.forEach(function(f){var e=est(f);if(e==='pendente'||e==='vencida')c[f.fornecedor]=(c[f.fornecedor]||0)+1;});
    ['TAM','GIT','BESTSELLER','CHLAMYS'].forEach(function(k){var el=document.getElementById('ag-b-'+k);if(el)el.textContent=c[k]||'';});
  }
  var FC={TAM:'#1565c0',GIT:'#6a1b9a',BESTSELLER:'#880e4f',CHLAMYS:'#2e7d32'};
  function rCharts(){
    var m={},total=0;
    ...
    if(svg&&lg){var cx=36,cy=36,r=27,sw=9,ci=2*Math.PI*r,off=0,paths='',lh='';Object.keys(m).filter(function(k){return m[k]>0;}).sort(function(a,b){return m[b]-m[a];}).forEach(function(k){var pct=total>0?m[k]/total:0,dash=ci*pct,gap=ci-dash,rot=-90+off*360;paths+='<circle cx="'+cx+'" cy="'+cy+'" r="'+r+'" fill="none" stroke="'+FC[k]+'" stroke-width="'+sw+'" stroke-dasharray="'+dash.toFixed(2)+' '+gap.toFixed(2)+'" transform="rotate('+rot.toFixed(1)+' '+cx+' '+cy+')"/>';lh+='<div class="ag-dl"><div class="ag-dl-dot" style="background:'+FC[k]+'"></div><span class="ag-dl-label">'+k+'</span><span class="ag-dl-pct">'+Math.round(pct*100)+'%</span></div>';off+=pct;});svg.innerHTML=paths+'<circle cx="'+cx+'" cy="'+cy+'" r="'+(r-sw/2-1)+'" fill="#fff"/>';lg.innerHTML=lh;}
  ...
  function rBanner(){
    ...
    var fnm=document.getElementById('ag-fb-name');fnm.textContent=agForn;fnm.style.color=FC[agForn]||'#000';
  ...
  function bRow(f,delay,seq){
    ...
    return'<tr class="'+tc+'" data-id="'+f.id+'"'+ds+'><td class="ag-thc ag-tdn">'+(seq!==undefined?seq:f.id)+'</td><td><span class="ag-chip ag-chip-'+f.fornecedor+'">'+f.fornecedor+'</span></td><td style="font-weight:bold">'+f.factura+'</td><td class="'+vc+'">'+fmt(f.valor)+'</td><td>'+fd(f.data)+'</td><td style="font-weight:bold">'+fd(f.vencimento)+'</td><td class="ag-thc">'+dh+'</td><td class="ag-thc">'+(em[e]||'')+'</td><td class="ag-thc">'+actions+'</td></tr>';
  }
  ...
  function rForn(){
    ...
    var keys=agForn?[agForn]:['TAM','GIT','BESTSELLER','CHLAMYS'].filter(function(k){return m[k];});
    container.className=agForn?'solo':'';
    container.innerHTML=keys.map(function(k){var d=m[k]||{p:0,e:0,t:0,n:0};return'<div class="ag-fb-block ag-fb-'+k+'"><div class="ag-fb-hdr"><span>'+k+'</span><span style="font-weight:bold">'+d.n+' fat.</span></div><div class="ag-fb-row"><span class="ag-fb-rl">pago</span><span class="ag-fb-rv g">'+fmt(d.p)+'</span></div><div class="ag-fb-row"><span class="ag-fb-rl">pendente</span><span class="ag-fb-rv'+(d.e>0?' r':'')+'">'+fmt(d.e)+'</span></div><div class="ag-fb-row" style="border-top:1px solid #e6e6e6"><span class="ag-fb-rl">total</span><span class="ag-fb-rv">'+fmt(d.t)+'</span></div></div>';}).join('');
  }
