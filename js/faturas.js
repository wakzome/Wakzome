...
(function () {
  ...
  function procAddRows(fid, n) {
    ...
    for (var i = 0; i < n; i++) {
      ...
      tr.innerHTML =
        ...
        + '<td><input type="number" min="0" step="1" maxlength="5" style="text-align:center !important;"'
        + ' oninput="procRecalcRow(' + f + ',' + r + ');procCheckAutoExpand(' + f + ',' + r + ');procLimitDigits(this,5)"></td>'
        + '<td><input type="number" min="0" step="1" maxlength="5"'
        + ' oninput="procRecalcRow(' + f + ',' + r + ');procCheckAutoExpand(' + f + ',' + r + ');procLimitDigits(this,5)"></td>'
        ...
        + '<td><input type="number" min="0" step="0.01" class="proc-preco-input" style="text-align:center !important;"'
        + ' onfocus="procActivateRow(this)"'
        + ' oninput="procRecalcRow(' + f + ',' + r + ');procCheckAutoExpand(' + f + ',' + r + ');procLimitDigits(this,5)"></td>'
  ...
  function procObterUltimoDiaVendasCarregado(callback) {
    ...
    modal.innerHTML =
      ...
      +     '<div class="proc-or-panel-title">'
      +       '<span class="proc-or-panel-title-main">' + fornecedorNorm + '</span>'
      +       '<style>@keyframes procUltimaDataPisca { 0%, 100% { opacity: 1; } 50% { opacity: .35; } }</style>'
      +       '<span class="proc-or-panel-title-sub">Peças compradas por referência e por ano · clique numa referência para ver o histórico<span id="proc-artigos-ultima-data" style="color:#8a6d1a;font-weight:700;animation:procUltimaDataPisca 1.1s ease-in-out infinite;"></span></span>'
      +     '</div>'
      +     '<button class="proc-or-close-btn">' + (aoVoltar ? '‹ Voltar' : '✕ Fechar') + '</button>'
      +   '</div>'
