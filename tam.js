...
(function () {
  ...
  function tamShowGuiaModal(invIdx) {
    ...
    function buildTableRows(rowList) {
      ...
      var isDone = rowList.length > 0 && rowList[0].done;
      // Build two independent column lists — no gaps regardless of distribution pattern
      var fList = rowList.filter(function(r){ return (isDone ? r.totalF : r.pendF) > 0; });
      var pList = rowList.filter(function(r){ return (isDone ? r.totalP : r.pendP) > 0; });
      var len = Math.max(fList.length, pList.length);
      if (len === 0) return '<tr><td colspan="5" class="tam-guia-empty">Sem referências pendentes</td></tr>';
      var html = '';
      for (var i = 0; i < len; i++) {
        var fr = fList[i] || null;
        var pr = pList[i] || null;
        var cls = isDone ? ' tam-guia-row-sent' : (i%2===0 ? ' tam-guia-row-even' : ' tam-guia-row-odd');
        var fRef = fr ? tamEsc(fr.ref) : '';
        var fQty = fr ? (isDone ? fr.totalF : fr.pendF) : 0;
        var pRef = pr ? tamEsc(pr.ref) : '';
        var pQty = pr ? (isDone ? pr.totalP : pr.pendP) : 0;
        html += '<tr class="tam-guia-tr' + cls + '">' +
          '<td class="tam-guia-td tam-guia-ref-f" data-gcol="0">' + fRef + '</td>' +
          '<td class="tam-guia-td tam-guia-qty-f" data-gcol="1">' + (fQty > 0 ? fQty : '') + '</td>' +
          '<td class="tam-guia-td tam-guia-sep"></td>' +
          '<td class="tam-guia-td tam-guia-ref-p" data-gcol="2">' + pRef + '</td>' +
          '<td class="tam-guia-td tam-guia-qty-p" data-gcol="3">' + (pQty > 0 ? pQty : '') + '</td>' +
        '</tr>';
      }
      return html;
    }
