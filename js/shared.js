...
(function(){
  ...
  //    "SEMANA N" numa única vista (2 linhas por pessoa, sem linha de totais) ──
  function renderFunchalUnificado(allBlocks, index){
    ...
    function buildSubTable(rows){
      ...
      persons.forEach(p=>{
        ...
        let rowspanCols=[];
        html+=`<tr class="${activeCls}">`;
        html+=`<td class="name" rowspan="2" style="background:${bg};width:${colWidths[0]*12}px;text-align:center;justify-content:center;">
                <span style="display:inline-block;width:10px;height:10px;border-radius:50%;background:${circleColor};margin-right:6px;vertical-align:middle;flex-shrink:0;"></span>
                ${escapeHtml(A[0]||'')}</td>`;
        for(let c=1;c<cols;c++){
          const cls=(c===todayCol?'today-col':'');
    ...
    document.getElementById('table-container').innerHTML = combined;
  }
